    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <?php
          ...
          $sql = "SELECT id, profile_picture, unm, no, age, rd1, role FROM loginsystem";
          $result = $con->query($sql);
          ...
          if ($result && $result->num_rows > 0) {
            ...
            // Display each user's data with Update and Delete buttons
            while($row = $result->fetch_assoc()) {
              echo "<tr>";
              echo "<td>" . htmlspecialchars($row['id']) . "</td>";
              echo "<td>" . htmlspecialchars($row['unm']) . "</td>";
              // Display profile picture as an image
              
              echo "<td class='text-center'><img src='uploads/" . htmlspecialchars($row['profile_picture']) . "' alt='Profile Picture' width='100' height='100'></td>";
              echo "<td>" . htmlspecialchars($row['no']) . "</td>";
              echo "<td>" . htmlspecialchars($row['age']) . "</td>";
              echo "<td>" . htmlspecialchars($row['rd1']) . "</td>";
              echo "<td>" . htmlspecialchars($row['role']) . "</td>";
              
              echo "<td class='text-center'>";
              echo "<a href='update_user.php?id=" . urlencode($row['id']) . "' class='btn btn-primary btn-sm col-lg-4 mb-0'>Update</a>";
              echo "<a href='admin_delete.php?id=" . urlencode($row['id']) . "' class='btn btn-danger btn-sm col-lg-4 mb-0 mt-0' onclick='return confirm(\"Are you sure you want to delete this user?\");'>Delete</a>";
              echo "<a href='disp-frd.php?id=" . urlencode($row['id']) . "' class='btn btn-primary btn-sm col-lg-4 mb-0'>View</a><br>";
              echo "</td>";
              echo "</tr>";
            }
            echo "</tbody></table>";
